...
def generar_reporte(comentarios, comentarios_pos, comentarios_neu, comentarios_neg):
    ...
    for comentario_pos in comentarios_pos:
        ...
        categorias = [cat.categoria for cat in comentario.categoria_comen]  # Obtener categorías relacionadas
    ...
    for index, row in df.iterrows():
        ...
        for cat in categorias_lista:
            # La categoría puede ser un objeto o un valor directo
            cat_nombre = cat
            if hasattr(cat, 'categoria'):  # Si es un objeto con atributo 'categoria'
    ...
    # Crear PDF con SimpleDocTemplate para mejor manejo de elementos
    ...
    # Crear estilo para comentarios en celdas
    comentario_style = ParagraphStyle(
        ...
        spaceAfter=6,
        wordWrap='CJK'  # Mejor manejo de wrapping para textos largos
    )
    ...
    # Tabla de categorías más frecuentes
    ...
    for cat, count in categorias_ordenadas[:10]:  # Mostrar las 10 categorías más frecuentes
    ...
    # NUEVAS SECCIONES - COMENTARIOS POSITIVOS
    ...
    for comentario_pos in comentarios_pos:
        ...
        # Corregido: Obtener categorías correctamente
        categorias = []
    ...
    if len(comentarios_pos_data) > 1:  # Si hay comentarios positivos
        pos_table = Table(
    ...
    if len(comentarios_neu_data) > 1:  # Si hay comentarios neutrales
        neu_table = Table(
    ...
    # COMENTARIOS NEGATIVOS (con palabra no censurada)
    ...
    for comentario_neg in comentarios_neg:
        ...
        # Corregido: Obtener palabra no censurada correctamente
    ...
    if len(comentarios_neg_data) > 1:  # Si hay comentarios negativos
        neg_table = Table(
    ...
    # Tabla detallada de todos los comentarios (la que ya existía)
    elements.append(Spacer(1, 20))
    ...
    # Llenar la tabla con los datos - usando Paragraph para texto largo
    for index, row in df.iterrows():
        # Limitar comentario a 300 caracteres y formatear como Paragraph
        comentario_text = row['comentario']
        if len(comentario_text) > 300:
            comentario_text = comentario_text[:297] + '...'
        
        # Usar Paragraph para manejar correctamente el texto largo
        comentario_paragraph = Paragraph(comentario_text, comentario_style)
        
        # Procesar categorías considerando diferentes posibilidades
        categorias_lista = row['categoria']
        categoria_textos = []
        
        for cat in categorias_lista:
            # La categoría puede ser un objeto con atributo 'categoria' o directamente un valor
            if hasattr(cat, 'categoria'):
    ...
    # Ajustar alturas de las filas y columnas para los comentarios largos
    comentarios_table = Table(
        comentarios_data, 
        colWidths=[40, 250, 150, 80],
        repeatRows=1  # Repetir la fila de encabezado en cada página
    )
    
    comentarios_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.darkblue),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
        ('ALIGN', (0, 0), (0, -1), 'CENTER'),  # Centrar IDs
        ('ALIGN', (2, 0), (3, -1), 'CENTER'),  # Centrar categorías y polaridad
        ('ALIGN', (1, 0), (1, 0), 'CENTER'),   # Centrar encabezado de comentarios
        ('ALIGN', (1, 1), (1, -1), 'LEFT'),    # Alinear comentarios a la izquierda
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, 0), 12),
        ('BOTTOMPADDING', (0, 0), (-1, 0), 12),
        ...
        ('LEFTPADDING', (0, 0), (-1, -1), 5),
        ('RIGHTPADDING', (0, 0), (-1, -1), 5),
        # Asegurar que la celda se adapte verticalmente al contenido
        ('ROWHEIGHT', (0, 1), (-1, -1), None),
        # Espaciado adicional para mejorar legibilidad
        ('BOTTOMPADDING', (0, 1), (-1, -1), 8),
        ('TOPPADDING', (0, 1), (-1, -1), 8),
    ]))
    
    # Colorear filas según la polaridad
